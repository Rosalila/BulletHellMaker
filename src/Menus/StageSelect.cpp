...
vector<string> getReplayInput(char *replay_data, int replay_size)
{
  ...
  for (; replay_iterator < replay_size && replay_data[replay_iterator] != '\n';
       replay_iterator++)
  {
...
void updateMusic(int current_stage)
{
  ...
  {
    if (rosalila()->sound->current_music != std::string(assets_directory) + "1.ogg")
      rosalila()->sound->playMusic(std::string(assets_directory) + "1.ogg", -1);
  }
  if (current_stage >= 12 && current_stage <= 29)
  {
    if (rosalila()->sound->current_music != std::string(assets_directory) + "2.ogg")
      rosalila()->sound->playMusic(std::string(assets_directory) + "2.ogg", -1);
  }
  if (current_stage >= 30 && current_stage <= 53)
  {
    if (rosalila()->sound->current_music != std::string(assets_directory) + "3.ogg")
      rosalila()->sound->playMusic(std::string(assets_directory) + "3.ogg", -1);
  }
  if (current_stage >= 54 && current_stage <= 58)
  {
    if (rosalila()->sound->current_music != std::string(assets_directory) + "4.ogg")
      rosalila()->sound->playMusic(std::string(assets_directory) + "4.ogg", -1);
  }
}
...
void stageSelect()
{
  ...
  while (true)
  {
    ...
    if (rosalila()->receiver->isDown(0, "6") && entry_navigator == 0)
    {
    ...
    if (rosalila()->receiver->isDown(0, "4") && entry_navigator == 0)
    {
    ...
    if (rosalila()->api_integrator->isUsingApi() && rosalila()->receiver->isDown(0, "2"))
    {
      ...
      {
        rosalila()->sound->playSound("Menu.down", 0, 0, 0);
        entry_navigator++;
        line_width = 0;
    ...
    if (rosalila()->api_integrator->isUsingApi() && rosalila()->receiver->isDown(0, "8"))
    {
      if (!current_leaderboard)
      {
        rosalila()->api_integrator->findLeaderboard(stage_names[current_stage]);
      ...
      {
        rosalila()->sound->playSound("Menu.up", 0, 0, 0);
        entry_navigator--;
        line_width = 0;
    ...
    if (rosalila()->receiver->isDown(0, "a") && select_button_was_up)
    {
      ...
      if (selected_entry != NULL)
      {
        ...
        while (selected_entry->attachment_state != "loaded" && rosalila()->api_integrator->getState() == "loading")
        {
          SDL_Delay(17);
          rosalila()->update();
        }
    ...
    if (frame % 60 >= 0 && frame % 60 < 30)
    {
      ...
      if (rosalila()->api_integrator->isUsingApi())
      {
        ...
        {
          rosalila()->graphics->draw2DImage(down_arrow,
