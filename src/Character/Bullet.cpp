...
void Bullet::playSound(int panning, bool uses_camera)
{
  ...
  {
    current_channel = rosalila()->sound->playSound(random_sounds[current_random_sound], sound_channel, 0, panning);
  }
}
...
void Bullet::playHitSound(int panning, bool uses_camera)
{
  ...
  {
    rosalila()->sound->playSound("bullet_hit." + name, sound_channel + 5, 0, panning);
  }
}
